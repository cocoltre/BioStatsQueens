...
model_stats$`Adjusted\\_$R^{2}$` <- round(model_stats$`Adjusted\\_$R^{2}$`, 2)
model_stats$`Std.Error` <- round(model_stats$`Std.Error`, 2)
...
n <- ncol(table1)
widths_vec <- rep(1, n)
widths_vec[1] <- 2  # Adjust this if "Model" is not first column
...
\vspace{-1em}
\parbox{\linewidth}{\fontsize{12}{14}\selectfont Table 6: Coefficients of Final Stepwise Model}
\end{minipage}
}
...
interaction_stats$`Adjusted\\_$R^{2}$` <- round(interaction_stats$`Adjusted\\_$R^{2}$`, 2)
interaction_stats$`Std.Error` <- round(interaction_stats$`Std.Error`, 2)
...
\vspace{-1em}
  ...
  \begin{minipage}[b]{0.3\linewidth}
    \centering
    {\fontsize{12}{14}\selectfont Figure 7: Residuals vs Observation Order plot\par}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.3\linewidth}
    \centering
