...
\vspace{0.5pt}
The aim of this report is to analyze the data and develop a
model to predict Mortgage Yield (in %) based on 6 explanatory
variables:\
...
group structures. As a result, the order of the predictors for the linear regression model does not influence the coefficient estimates, F-tests, or model interpretation.
...
# 2️⃣ Format numerics to 2 decimals with trailing zeroes
anova_table <- as.data.frame(lapply(anova_table, function(x) {
  if (is.numeric(x)) sprintf("%.2f", x) else x
}))

# 3️⃣ Generate the table grob
table <- gridExtra::tableGrob(anova_table, rows = NULL)

# 4️⃣ Apply white background & font size 12pt for all cells
for (i in seq_along(table$grobs)) {
  if (is.null(table$grobs[[i]]$gp)) table$grobs[[i]]$gp <- gpar()
  table$grobs[[i]]$gp$fill <- "#ffffff"
  table$grobs[[i]]$gp$fontsize <- 10
}

# 6️⃣ Add horizontal lines between rows
for (j in 1:(ncol(table) - 1)) {
  table <- gtable::gtable_add_grob(
    table,
    grobs = grid::linesGrob(
      x = unit(c(0, 0), "npc"),   # vertical line at left edge of column
      y = unit(c(0, 1), "npc"),
      gp = gpar(col = "black", lwd = 1)
    ),
    t = 1, b = nrow(table), l = j + 1
  )
}

# 7️⃣ Add full black border around entire table
table <- gtable::gtable_add_grob(
  table,
  grobs = grid::rectGrob(gp = gpar(col = "black", fill = NA, lwd = 1.2)),
  t = 1, l = 1, b = nrow(table), r = ncol(table)
)

# 8️⃣ Compute exact size (in inches) for perfect PNG export
w <- convertWidth(sum(table$widths), "in", valueOnly = TRUE)
h <- convertHeight(sum(table$heights), "in", valueOnly = TRUE)

# 9️⃣ Export PNG exactly to fit the table (no extra white space)
png("figures/Table 2.png", width = w, height = h, units = "in", res = 300, bg = "white")
...
model_summary <- as.data.frame(lapply(model_summary, function(x) {
  if (is.numeric(x)) sprintf("%.2f", x) else x
}))
...
png("figures/full_model_summary_table.png", width = w, height = h, units = "in", res = 300, bg = "white")
grid.newpage()
...
```{r}
model_stats <- broom::glance(model)[, c("r.squared", "adj.r.squared", "sigma", "statistic","df", "p.value")]
...
# Calcul taille PNG selon contenu
w <- convertWidth(sum(table2$widths), "in", valueOnly = TRUE)
h <- convertHeight(sum(table2$heights), "in", valueOnly = TRUE)

w <- w + 0.01
h <- h + 0.01

# Export PNG ajusté au contenu
png("figures/full_model_fitstats_table.png", width = w, height = h, units = "in", res = 300, bg = "white")
grid.draw(table2)
...
# Format numeric columns to 2 decimals for all numeric columns
step_summary <- as.data.frame(lapply(step_summary, function(x) {
  if(is.numeric(x)) sprintf("%.2f", x) else x
}))
...
ggsave("figures/stepwise_coef_table.png", table2, width = 4.5, height = 2, dpi = 300)
```
...
interaction_coef <- as.data.frame(lapply(interaction_coef, function(x) {
  if(is.numeric(x)) sprintf("%.2f", x) else x
}))
...
interaction_stats <- as.data.frame(lapply(interaction_stats, function(x) sprintf("%.2f", x)))
...
# Export PNG ajusté au contenu
png("figures/interaction_model_fitstats.png", width = w, height = h, units = "in", res = 300, bg = "white")
grid.draw(table2)
...
\includegraphics[width=1\linewidth]{figures/interaction_model_fitstats.png}\par
\fontsize{12}{14}\selectfont Table 11: Fit Statistics of Interaction Model\par
\addtocounter{table}{2}
...
par(mfrow = c(1, 3),        
    mar = c(3.5, 3.5, 2, 1),
    mgp = c(2, 0.6, 0),     
    cex.main = 1.2,     # Title size increased
    cex.lab = 1.2,      # Axis label size increased
    cex.axis = 1.0)     # Tick mark size increased
...
text(min(fitted(model)), max(resid(model))* 0.9, 
     labels = paste0("y = ", round(coef_res_fit[2], 2), "x + ", round(coef_res_fit[1], 2)), 
...
text(1, max(resid(model))* 0.9, 
     labels = paste0("y = ", round(coef_res_obs[2], 2), "x + ", round(coef_res_obs[1], 2)), 
     pos = 4, col = "red", cex = 1.2)
...
# Conclusion
The final estimated model is : $\hat{mortYld}$ = 4.22 + 0.02$\cdot$**X1** -
0.002$\cdot$**X3** + 0.0002$\cdot$**X4**\
